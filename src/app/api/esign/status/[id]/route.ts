...
export async function GET(
  ...
  try {
        ...
        try {
          ...
          if (docusealRes.ok) {
            ...
            if (liveDocusealData.submitters) {
              signers = liveDocusealData.submitters.map((sub: { name: string; email: string; role?: string; status?: string; opened_at?: string | null; completed_at?: string | null }) => ({
                name: sub.name,
                email: sub.email,
                role: sub.role,
    ...
    return NextResponse.json(
      { success: false, error: (err instanceof Error ? err.message : 'Failed to retrieve envelope status') },
      { status: 500 }
    );
  }
}
